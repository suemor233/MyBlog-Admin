...
import { FormInst, NButton, NIcon, NInput, NSpace, useMessage } from 'naive-ui'
  ...
  async setup(props, ctx) {
    ...
    const handleValidateButtonClick = (e: MouseEvent) => {
      ...
      formRef.value?.validate(async (errors) => {
        ...
        if (isPublic.value) {
          toast.success('发布成功')
          await router.push('/posts/view')
        } else {
          toast.success('保存成功')
          await router.push({ path: '/posts/edit', query: { id: res.data.id } })
        }
      })
    }
    ...
    onMounted(async () => {
      if (route.query.id) {
        const res = await appStore.useArticle.getArticleById(
          route.query.id as string,
        )

        if (!res.success || !res.data) {
          toast.error('文章不存在')
          await router.push('/posts/edit')
          return
        }
    ...
    return () => (
      <>
        <ContentLayout
          title={`博文 · ${
            router.currentRoute.value.query.id ? '修改' : '撰写'
          }`}
          v-slots={slots}
        >
          <div class={classes.myMarkdown}>
            <NInput
              class={classes.mInput}
              v-model:value={articleForm.title}
              type={'text'}
              placeholder={'在这里输入主人的文章标题'}
            />
            <MyEditor
              {...{
                onHandleSave: handleSave,
                onHandleText: (msg) => {
                  articleForm.content = msg
                },
              }}
            />
          </div>
          <MyArticleDialog
            ...
            {...{ onHandleValidateButtonClick: handleValidateButtonClick }}
          />
        </ContentLayout>
      </>
    )
